...
from .events import Event, Goal, KickOff, MatchEnded, MatchEvent, MatchStarted, RedCard, Substitution, YellowCard


class MatchSimulator:
    """Deterministic football match simulator."""
    
    def __init__(self, world: GameWorld, match: Match, seed: Optional[int] = None) -> None:
        self.world = world
        self.match = match
    ...
    def _create_event(self, event_type: str) -> Optional[MatchEvent]:
    ...
    def _create_goal_event(self) -> Goal:
        ...
        attackers = [p for p in scoring_team.players if p.position in [Position.ST, Position.LW, Position.RW, Position.CAM]]
        if not attackers:
            attackers = [p for p in scoring_team.players if p.position not in [Position.GK]]
        ...
        if self.rng.random() < 0.6:  # 60% chance of assist
            possible_assists = [p for p in scoring_team.players if p.id != scorer.id and p.position != Position.GK]
            if possible_assists:
    ...
    def _create_yellow_card_event(self) -> YellowCard:
        """Create a yellow card event."""
        team = self.rng.choice([self.home_team, self.away_team])
        player = self.rng.choice(team.players)
        
        reasons = ["Unsporting behavior", "Dissent", "Persistent fouling", "Delaying the game", "Simulation"]
    ...
    def _create_red_card_event(self) -> RedCard:
        ...
        reasons = ["Serious foul play", "Violent conduct", "Second yellow card", "Offensive language"]
        reason = self.rng.choice(reasons)
    ...
    def _create_substitution_event(self) -> Optional[Substitution]:
...
class MatchEngine:
    ...
    def simulate_match(self, match_id: str, seed: Optional[int] = None) -> List[MatchEvent]:
